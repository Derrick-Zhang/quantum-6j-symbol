...
BeginPackage["HOMFLYPTinvariants`"]
...
Begin["`TwistFactor`"]
  ...
  arm := HOMFLYPTinvariants`CommonDefinition`arm;
  leg := HOMFLYPTinvariants`CommonDefinition`leg;
  coarm := HOMFLYPTinvariants`CommonDefinition`coarm;
  coleg := HOMFLYPTinvariants`CommonDefinition`coleg;
  MoL := HOMFLYPTinvariants`CommonDefinition`MoL;
  bra := HOMFLYPTinvariants`CommonDefinition`bra;
  Variable:=  HOMFLYPTinvariants`CommonDefinition`Variable;
  StringPartGenerate :=  HOMFLYPTinvariants`CommonDefinition`StringPartGenerate;
  Per := HOMFLYPTinvariants`CommonDefinition`Per;
...
Table[QBinomial[$\[Lambda][[i]]+$m-i,$\[Mu][[j]]+$m-j,q^(2)],{i,1,$m},{j,1,$m}]
  ...
  Evaluateblock[\[Lambda]_,m_,s_]:=Module[{mm=m,i,result="",addterm},
      For[i=1,i<=mm-1,i++,
           addterm = "HOMFLYPTinvariants`TwistFactor`block["<>ToString[s]<>",(HOMFLYPTinvariants`TwistFactor`GenDiagramList["<>ToString[\[Lambda]]<>","<>ToString[mm]<>"])\[LeftDoubleBracket]"<>ToString[i]<>",1"<>StringPartGenerate[k,i-1]<>"\[RightDoubleBracket]"<>",HOMFLYPTinvariants`TwistFactor`GenDiagramList["<>ToString[\[Lambda]]<>","<>ToString[m]<>"]\[LeftDoubleBracket]"<>ToString[i+1]<>",1"<>StringPartGenerate[k,i]<>"\[RightDoubleBracket]]";
           result = addterm<>result;
         ];
      Return[result<>"HOMFLYPTinvariants`TwistFactor`d["<>ToString[s]<>",(HOMFLYPTinvariants`TwistFactor`GenDiagramList["<>ToString[\[Lambda]]<>","<>ToString[mm]<>"])\[LeftDoubleBracket]"<>ToString[mm]<>",1"<>StringPartGenerate[k,mm-1]<>"\[RightDoubleBracket]]"];
];
  
  IteratorGenerator[\[Lambda]_,m_]:=Module[{mm=m,$\[Lambda] = \[Lambda]},
    Table[{ToString[Variable[k,i]]<>",Length@((HOMFLYPTinvariants`TwistFactor`GenDiagramList["<>ToString[$\[Lambda]]<>","<>ToString[mm]<>"])\[LeftDoubleBracket]"<>ToString[i+1]<>",1"<>StringPartGenerate[k,i-1]<>"\[RightDoubleBracket])"},{i,1,mm-1}]
 ];
...
reEvaluateblock[\[Lambda]_,m_,s_]:=Module[{mm=m,i,result="",addterm},
      For[i=1,i<=mm-1,i++,
           addterm = "HOMFLYPTinvariants`TwistFactor`reblock["<>ToString[s]<>",(HOMFLYPTinvariants`TwistFactor`reGenDiagramList["<>ToString[\[Lambda]]<>","<>ToString[mm]<>"])\[LeftDoubleBracket]"<>ToString[i]<>",1"<>StringPartGenerate[k,i-1]<>"\[RightDoubleBracket]"<>",HOMFLYPTinvariants`TwistFactor`reGenDiagramList["<>ToString[\[Lambda]]<>","<>ToString[mm]<>"]\[LeftDoubleBracket]"<>ToString[i+1]<>",1"<>StringPartGenerate[k,i]<>"\[RightDoubleBracket]]";
           result = addterm<>result;
         ];
      Return[result<>"HOMFLYPTinvariants`TwistFactor`dref["<>ToString[s]<>",(HOMFLYPTinvariants`TwistFactor`reGenDiagramList["<>ToString[\[Lambda]]<>","<>ToString[mm]<>"])\[LeftDoubleBracket]"<>ToString[mm]<>",1"<>StringPartGenerate[k,mm-1]<>"\[RightDoubleBracket]]"];
];

reIteratorGenerator[\[Lambda]_,m_]:=Module[{mm=m,$\[Lambda] = \[Lambda]},
    Table[{ToString[Variable[k,i]]<>",Length@((HOMFLYPTinvariants`TwistFactor`reGenDiagramList["<>ToString[$\[Lambda]]<>","<>ToString[mm]<>"])\[LeftDoubleBracket]"<>ToString[i+1]<>",1"<>StringPartGenerate[k,i-1]<>"\[RightDoubleBracket])"},{i,1,mm-1}]
];
...
Begin["`HOMFLYPT`"]
   ...
   d:=HOMFLYPTinvariants`TwistFactor`d;
   UnrefinedTWFactor:=HOMFLYPTinvariants`TwistFactor`UnrefinedTWFactor;
     Per := HOMFLYPTinvariants`CommonDefinition`Per;
     arm := HOMFLYPTinvariants`CommonDefinition`arm;
     leg := HOMFLYPTinvariants`CommonDefinition`leg;
     coarm := HOMFLYPTinvariants`CommonDefinition`coarm;
...
Begin["`PoincarePolynomial`"]
     ...
     dref:=HOMFLYPTinvariants`TwistFactor`dref;
     RefinedTwFactor:=HOMFLYPTinvariants`TwistFactor`RefinedTwFactor;
     PerdZ := HOMFLYPTinvariants`TwistFactor`PerdZ;
     arm := HOMFLYPTinvariants`CommonDefinition`arm;
     leg := HOMFLYPTinvariants`CommonDefinition`leg;
     coarm := HOMFLYPTinvariants`CommonDefinition`coarm;
     coleg := HOMFLYPTinvariants`CommonDefinition`coleg;
